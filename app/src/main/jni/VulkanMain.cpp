...
uint32_t findMemoryType(uint32_t typefilter, VkMemoryPropertyFlags properties)
{
    ...
    __android_log_print(ANDROID_LOG_ERROR, "findmemorytype error ",               \
                        "Vulkan error. File[%s], line[%d]", __FILE__, \
                        __LINE__);                                    \
    assert(false);
}
...
void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
    ...
    CALL_VK(vkCreateBuffer(device.device_, &bufferInfo, nullptr, &buffer));
...
{
    VkDeviceSize bufferSize = sizeof(uniBuffObj);
...
bool CreateBuffers(void) {
    ...
    // Vertex positions
        ...
        CALL_VK(vkCreateBuffer(device.device_, &createBufferInfo, nullptr,
                               &buffers.vertexBuf_[0]));

        VkMemoryRequirements memReq;
        vkGetBufferMemoryRequirements(device.device_, buffers.vertexBuf_[0], &memReq);
...
bool CreateBuffers2(void) {
    // -----------------------------------------------
    // Create the triangle vertex buffer

    // Vertex positions
    const float vertexData[] = {
            0.5f, -0.5f, 0.5f, 1.5f, -1.5f, 0.5f, -0.4f, 1.3f, 0.0f,
    };
    ...
    CALL_VK(vkCreateBuffer(device.device_, &createBufferInfo, nullptr,
                           &buffers.vertexBuf_[1]));

    VkMemoryRequirements memReq;
    vkGetBufferMemoryRequirements(device.device_, buffers.vertexBuf_[1], &memReq);
    ...
    CALL_VK(
            vkBindBufferMemory(device.device_, buffers.vertexBuf_[1], deviceMemory, 0));
...
void DeleteBuffers(void) {
    vkDestroyBuffer(device.device_, buffers.vertexBuf_[0], nullptr);
}
...
void readFile(const char* filePath)
{
    ...
    {
        if (fileContent[i] == 'v' && fileContent[i+1] == ' ')
        {
            i+=2;
            objVerts.push_back((float)atof(&fileContent[i])); //Push the x coordinates
            ...
            i++;
            objVerts.push_back((float)atof(&fileContent[i]));//push the y coordinates
...
bool InitVulkan(android_app* app) {
    ...
    for (int bufferIndex = 0; bufferIndex < swapchain.swapchainLength_;
        ...
        vkCmdBindDescriptorSets(render.cmdBuffer_[bufferIndex], VK_PIPELINE_BIND_POINT_GRAPHICS, gfxPipeline.layout_, 0, 1, &descriptorSet, 0,
                                nullptr);
...
bool VulkanDrawFrame(void) {
    ...
    vkCmdPushConstants(render.cmdBuffer_[nextIndex],gfxPipeline.layout_,VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                       sizeof(UniFrag), &sun);
...
void updateUniformBuffer()
{
    ...
    void* data;
    vkMapMemory(device.device_, uniformBufferMemory, 0, sizeof(mvp), 0, &data);
    memcpy(data, &mvp, sizeof(mvp));
    vkUnmapMemory(device.device_, uniformBufferMemory);
...
void createDescriptorPool()
{
    ...
    CALL_VK(vkCreateDescriptorPool(device.device_, &poolInfo, nullptr, &descriptorPool));
}
...
void createDescSet()
{
    ...
    bufferInfo.offset = 0;
    bufferInfo.range = sizeof(uniBuffObj);
    ...
    vkUpdateDescriptorSets(device.device_, 1, &descriptorWrite, 0, nullptr);
...
void initialiseuniforms()
{
    ...
    mvp.proj [0] = 2.0f/(-1.0f-1.0f);
    mvp.proj [3] = -(-1.0f-1.0f/-1.0f-1.0f);
    mvp.proj [5] = 2.0f/(-1.0f-1.0f);
    mvp.proj [7] = -(-1.0f + 1.0f/-1.0f - 1.0f);
    mvp.proj [10] = 2.0f/(-5.0f - 5.0f);
